...
logs_view() {
  kcv_init
  echo "  1) llama-server  2) rpc-server  3) wireguard  4) ollama  5) health  6) backup"
  local r n=100
  read -r -p "select: " r
  case "$r" in
...
cron_menu_run() {
  ...
  case "$r" in
    ...
    4) cron_remove health; cron_remove backup; cron_remove ollama-patch ;;
    *) : ;;
  esac
}
